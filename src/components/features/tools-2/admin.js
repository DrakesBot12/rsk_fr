                            ...
                            links.map((link, index) => (
                                ...
                                <div key={index} className='grid grid-cols-[auto_1fr_2fr_1.5fr_1fr_auto] items-center gap-3'>
                                    <Input
                                        type="number"
                                        value={link.order || ''}
                                        onChange={e => onLinkChange(type.key, index, 'order', e.target.value)}
                                        placeholder="№"
                                    ...
                                    <Input 
                                        value={link.name} 
                                        onChange={e => onLinkChange(type.key, index, 'name', e.target.value)} 
                                        placeholder="Название сервиса"
                                    />
                                    <Input 
                                        value={link.url} 
                                        onChange={e => onLinkChange(type.key, index, 'url', e.target.value)} 
                                        placeholder="URL-адрес" 
                                    />
                                    <Input 
                                        value={link.description || ''} 
                                        onChange={e => onLinkChange(type.key, index, 'description', e.target.value)} 
            ...
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
                    ...
                    <form onSubmit={handleLogin} className="space-y-4">
                        ...
                        <Button type="submit" className="w-full !py-3 !text-base blue">Войти</Button>
                        <Button onClick={() => goTo('trainer')} className="w-full !py-3 !text-base">Вернуться на главную</Button>
                    </form>
                </div>
            </div>
        );
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            <Header>
                ...
                <Header.Heading className="mb-4 text-center text-xl md:text-2xl">
    Админ-панель: Сервисы и Ссылки
</Header.Heading>
<Button onClick={() => goTo('trainer')} className="w-full">
    Вернуться к тренажеру
            ...
            <main className="container mx-auto py-8 px-4">
                <Block className="!p-6">
                    {constants && constants.defaultTypes.map((type) => {
                ...
                <div className='mt-8'>
                    <Button onClick={handleSave} className='blue w-full text-lg !py-3 shadow-lg hover:shadow-xl transition-shadow'>Сохранить все изменения</Button>
