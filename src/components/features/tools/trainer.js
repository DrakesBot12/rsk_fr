  ...
  const [confirmationConfig, setConfirmationConfig] = useState({
	...
	const toggleTaskTimer = async () => {
		if (isTaskRunning) {
			...
			try {
				...
				const taskTextData = tasksText.find(
					t =>
	...
	const handleSaveSessionCompletion = async levels => {
		try {
			...
			const response = await fetch('/api/saveDeltaTest', {
				method: 'POST',
				headers: {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <input
                ...
                max="10"
                value={aiLevelNow}
                onChange={(e) => setAiLevelNow(parseInt(e.target.value))}
                className="w-full"
              />
              <span className="text-sm w-24">10 (Эксперт)</span>
            </div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              0 = Все равно ничего не понятно.<br />
              5 = Теперь я понимаю, как решать простые рабочие задачи с помощью 2-3 инструментов.<br />
          ...
          <div>
            <label className="block mb-2 font-medium">
              2. Какими из перечисленных ИИ-инструментов вы ТЕПЕРЬ умеете пользоваться?
            </label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {toolsList.map(category => (
                <div key={category.category} className="border p-3 rounded-lg">
                  ...
                  <div className="space-y-2">
                    {category.tools.map(tool => (
                      <label key={tool} className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={selectedToolsNow.includes(tool)}
                          onChange={() => handleToolToggle(tool)}
                        />
                        {tool}
                      </label>
              ...
              {[
                ...
                <label key={option} className="flex items-center gap-2">
                  <input
                    type="radio"
                    name="attitudeChange"
                    checked={attitudeChange === option}
                    onChange={() => setAttitudeChange(option)}
                  />
                  {option}
                </label>
              ))}
          ...
          <div>
            <label className="block mb-2 font-medium">
              4. Какой главный вывод о командной работе вы сделали за время тренажера?
            </label>
            <textarea
              value={teamworkConclusion}
              onChange={(e) => setTeamworkConclusion(e.target.value)}
              className="w-full p-3 border border-gray-300 rounded-md h-24"
              placeholder="Опишите ваш вывод..."
            />
          </div>
...
const saveQuestionnaire = async (questionnaireType, data) => {
  try {
    ...
    const response = await fetch('/api/saveQuestionnaire', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    <div className='col-span-12 mb-4'>
        <div className='flex flex-col gap-[1.6rem]'>
            <div className='flex gap-[1.6rem]'>
                ...
                <div className='flex gap-[0.5rem]'>
                    <Button
                        ...
                        onClick={showCompleteSessionConfirmation}
                    >
                        Завершить&nbsp;сессию
                    </Button>
                </div>
            ...
            <div className='flex gap-[0.5rem]'>
                ...
                <Switcher value={who} onChange={(value) => {
                    setWho(value);
                    if (value === 'we') {
                        handleSwitchToWe();
                    }
            ...
            <div className='flex flex-col gap-[0.75rem]'>
                ...
                <div className='flex flex-wrap lg:flex-nowrap gap-[0.5rem] items-center'>
                    <Button
                        className={
                            isTaskRunning
                                ? '!bg-(--color-red-noise) !text-(--color-red)'
                                : '!bg-(--color-green-noise) !text-(--color-green-peace)'
                        }
                        onClick={toggleTaskTimer}
                    >
                        {isTaskRunning
                            ? `Завершить задание (${formatTaskTime(taskElapsedTime)})`
                            : 'Начать задание'}
                    </Button>

                    {taskFileUrl && (
                        <Button as='a' href={taskFileUrl} download onClick={e => { e.preventDefault(); window.open(taskFileUrl, '_blank'); }} d>
                            Доп.материал
                        </Button>
                    )}
                    {instructionFileUrl && (
                        <Button as='a' href={instructionFileUrl} download onClick={e => { e.preventDefault(); window.open(instructionFileUrl, '_blank'); }} d>
                            Инструкция
                        </Button>
                    )}
                    {currentTask?.toolLink1 && (
                        <Button inverted as='a' href={currentTask.toolLink1} target='_blank' onClick={e => { if (currentTaskIndex + 1 === 1) { e.preventDefault(); setShowFirstQuestionnaire(true); } else if (currentTaskIndex + 1 === 2) { e.preventDefault(); window.open('https://forms.yandex.ru/u/689197c9eb6146293aca92fa/', '_blank'); } else { e.preventDefault(); window.open(currentTask.toolLink1, '_blank'); } }}>
                            {currentTask.toolName1 || 'Инструмент'}
                        </Button>
                    )}
                    {currentTask?.toolLink2 && (
                        <Button inverted as='a' href={currentTask.toolLink2} target='_blank' onClick={e => { e.preventDefault(); window.open(currentTask.toolLink2, '_blank'); }}>
...
<Button icon type="button" onClick={() => handleRandom(f.code)}>
    <RandomIcon />
										...
										<>
											...
											<Button
												...
												onClick={() => handleShowBufferForField(f.code)}
												type='button'
											>
												<CopyIcon />
											</Button>
											<Button
												icon
												className='!hidden group-hover:!flex'
												onClick={() => handleAddToBuffer(f.code)}
												type='button'
											>
												<Plusicon />
											</Button>
								...
								{fieldsList.slice(4).map(f => (
									...
									{isMobile ? (
										<>
											...
											<div className='flex flex-shrink-0 items-center gap-2'>
    <Button icon type="button" onClick={() => handleShowBufferForField(f.code)}><CopyIcon /></Button>
    <Button icon type="button" onClick={() => handleAddToBuffer(f.code)}><Plusicon /></Button>
    <Button icon type="button" onClick={() => handleRandom(f.code)}><RandomIcon /></Button>
</div>
										</>
									) : (
										<>
    ...
    title={isCreateDisabled ? 'Сначала заполните все поля' : ''}
>
    <Button
			...
			<div className='bg-white p-6 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto'>
				<div className='flex justify-between items-start mb-4'>
					<h3 className='text-xl font-bold'>
						Задание завершено за {formatTaskTime(elapsedTime)}
					</h3>
					<button
						onClick={onClose}
						className='square text-gray-500 hover:text-gray-700'
...
async function saveToJson({
	...
	try {
		...
		const response = await fetch('/api/saveTaskAttempt', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
