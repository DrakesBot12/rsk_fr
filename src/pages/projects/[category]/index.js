...
export default function CategoryPage() {
  ...
  const completedCount = projectsInCategory.filter((p) =>
    p.tasks.every((t) => t.status === "SUCCESS")
  ).length;
  ...
  const sorted = [
    ...sortedBase.filter((p) => !p.tasks.every((t) => t.status === "SUCCESS")),
    ...sortedBase.filter((p) => p.tasks.every((t) => t.status === "SUCCESS")),
  ];
      ...
      <div className="hero" style={{ gridTemplateRows: "max-content" }}>
        <div className="col-span-12 flex items-start justify-between">
          ...
          <Card>
            <Card.Heading>
              <h5>
                Вы выполнили <span className="text-(--color-blue)">{completedCount}</span> проекта из{" "}
                <span className="text-(--color-blue)">{total}</span>
              </h5>
        ...
        <div className="col-span-12 grid grid-cols-3 gap-[1.25rem] h-fit">
          {sorted.map((project, idx) => (
            <div
              ...
              className="flex flex-col min-h-[200px] justify-between gap-[1rem] p-[1.25rem] rounded-[1rem] border-[1.5px] border-(--color-gray-plus-50) hover:bg-(--color-white-gray) hover:border-(--color-white-gray) transition"
            >
              ...
              {idx === 0 ? (
                // Первый проект ведет на страницу курса
                <Link href={`/projects/${url}/aicourses`}>
                  <div
                    className={`flex items-center justify-center px-[.75rem] py-[1rem] rounded-[6.25rem] ${
                      project.tasks.every((t) => t.status === "SUCCESS")
                        ? "bg-(--color-green-noise) text-(--color-green-black)"
                        : "bg-(--color-gray-plus-50) text-(--color-black)"
                    }`}
                  >
